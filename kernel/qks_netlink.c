...
static int qks_cmd_verdict(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (!ok) {
        qks_log_sig_invalid(ev->event_id, "signature_verification_failed");
    ...
    qks_log_hash_verified(ev->event_id);
    qks_log_sig_valid(ev->event_id);
    
    /* Log the verdict decision before applying it */
    const char *verdict_str = (v->verdict == QKS_DENY) ? "DENY" : "ALLOW";
    qks_log("[KERNEL] Verdict decision: %s | event_id=%llu | reason=%s", 
            verdict_str, (u64)ev->event_id, v->reason ? v->reason : "none");
